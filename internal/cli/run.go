...
// RunProfile executes the profile subcommand: parse the flags, load and
// validate the scenario, run the solver and print the report. Any
// illegal input surfaces as a non-nil error, which the main function
// turns into a stderr line and a non-zero exit code.
...
// writeCSV persists the profile dump to a file. It replaces an existing
// file only after a successful encode, so a failed run never leaves a
